defmodule ShardWeb.UserLive.NpcCommandsTest do
  ...
  describe "execute_talk_command/2" do
    ...
    test "shows available quests when NPC has quests", %{
      game_state: game_state,
      npc: npc,
    ...
    test "shows turn-in dialogue for completed quests", %{
      game_state: game_state,
      npc: npc,
      character: character
    } do
  ...
  describe "execute_deliver_quest_command/2" do
    ...
    test "returns error when NPC not found", %{game_state: game_state} do
      {messages, _state} =
        NpcCommands.execute_deliver_quest_command(game_state, "Nonexistent NPC")
    ...
    test "handles case-insensitive NPC name matching", %{game_state: game_state, npc: npc} do
      ...
      {messages1, _} =
        NpcCommands.execute_deliver_quest_command(game_state, String.upcase(npc.name))

      {messages2, _} =
        NpcCommands.execute_deliver_quest_command(game_state, String.downcase(npc.name))
