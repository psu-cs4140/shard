defmodule Shard.PayoutTest do
  ...
  setup do
    user =
      Shard.Repo.insert!(%Shard.Users.User{email: "test@example.com", hashed_password: "password"})

    character =
      Shard.Repo.insert!(%Character{
        user_id: user.id,
        name: "Gambler",
        gold: 1000,
        level: 1,
  ...
  test "process_payouts handles wins correctly", %{character: character} do
    ...
    # 20
    player_cards = [%{rank: "K", suit: "hearts"}, %{rank: "Q", suit: "hearts"}]
    Repo.update!(Ecto.Changeset.change(hand, %{hand_cards: player_cards, status: "playing"}))

    # 18
    dealer_cards = [%{rank: "10", suit: "spades"}, %{rank: "8", suit: "clubs"}]
  ...
  test "process_payouts handles pushes correctly", %{character: character} do
    ...
    # 20
    dealer_cards = [%{rank: "10", suit: "spades"}, %{rank: "Q", suit: "clubs"}]
