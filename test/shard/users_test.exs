defmodule Shard.UsersTest do
  ...
  describe "apply_user_email/3" do
    ...
    test "requires email to change", %{user: user} do
      {:error, changeset} = Users.apply_user_email(user, "invalid", %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{user: user} do
      {:error, changeset} =
        Users.apply_user_email(user, "valid", %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{user: user} do
      ...
      {:error, changeset} =
        Users.apply_user_email(user, "valid", %{email: too_long})
    ...
    test "validates email uniqueness", %{user: user} do
      ...
      {:error, changeset} =
        Users.apply_user_email(user, "valid", %{email: email})
    ...
    test "applies the email without persisting it", %{user: user} do
      email = unique_user_email()
      {:ok, user} = Users.apply_user_email(user, "valid", %{email: email})
      assert user.email == email
  ...
  describe "confirm_user/1" do
    ...
    test "confirms the email with a valid token", %{user: user, token: token} do
      ...
      assert Repo.get!(Users.User, user.id).confirmed_at
      refute Repo.get_by(Users.UserToken, user_id: user.id)
    end

    test "does not confirm with invalid token", %{user: user} do
      assert {:error, :transaction_aborted} = Users.confirm_user("oops")
      refute Repo.get!(Users.User, user.id).confirmed_at
      assert Repo.get_by(Users.UserToken, user_id: user.id)
    end

    test "does not confirm email if token expired", %{user: user, token: token} do
      {1, nil} = Repo.update_all(Users.UserToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      assert {:error, :transaction_aborted} = Users.confirm_user(token)
      refute Repo.get!(Users.User, user.id).confirmed_at
      assert Repo.get_by(Users.UserToken, user_id: user.id)
    end
  end
