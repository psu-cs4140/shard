defmodule Shard.UsersTest do
  ...
  describe "register_user/1" do
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} =
        Users.register_user(%{email: String.upcase(email), password: valid_user_password()})
  ...
  describe "zone progress" do
    ...
    test "unlock_next_zone/2 unlocks the next zone in sequence", %{user: user} do
      ...
      # Refresh the zone2 progress from database
      zone2_progress = Users.get_user_zone_progress(user.id, zone2.id)

      # The unlock function should have changed the progress
      case result do
        {:ok, updated_progress} when is_struct(updated_progress) ->
          assert updated_progress.progress == "in_progress"

        {:ok, :no_next_zone} ->
          # If there's no next zone, that's also valid
          assert true

        _ ->
          # For any other success case, verify the progress was updated
          assert zone2_progress.progress == "in_progress"
      end
    end

    test "unlock_next_zone/2 returns :no_next_zone when no next zone exists", %{
      user: user,
      zone: zone
    } do
  ...
  describe "sudo_mode?/2" do
    ...
    test "accepts custom time limit" do
      ...
      # 10 minutes ago
      assert Users.sudo_mode?(user, -10)
      # 2 minutes ago
      refute Users.sudo_mode?(user, -2)
    end
  end
end
