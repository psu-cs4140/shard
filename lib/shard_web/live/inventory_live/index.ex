defmodule ShardWeb.InventoryLive.Index do
  ...
  def handle_event("drop_item", %{"inventory_id" => inventory_id}, socket) do
    ...
    case Items.drop_item_in_room(character.id, String.to_integer(inventory_id), character.location) do
      {:ok, _} ->
        socket = 
          socket
  ...
  def handle_event("set_hotbar", %{"inventory_id" => inventory_id, "slot" => slot}, socket) do
    ...
    case Items.set_hotbar_slot(character.id, String.to_integer(slot), String.to_integer(inventory_id)) do
      {:ok, _} ->
        socket = 
          socket
